 ...
 * using precomputation of necessary group elements.
 */
...
fn pre_compute_group_elements (
    ik_b: EdwardsPoint,
    spk_b: EdwardsPoint,
    opk_b_opt: Option<EdwardsPoint>,
) -> Vec<EdwardsPoint> {
    let mut result: Vec<EdwardsPoint> = Vec::with_capacity(16);

    result.push(EdwardsPoint::identity());
    result.push(spk_b);

    result.push(ik_b);
    result.push(spk_b + ik_b);

    result.push(result[0] + spk_b);
    result.push(result[1] + spk_b);
    result.push(result[2] + spk_b);
    result.push(result[3] + spk_b);

    if let Some(opk_b) = opk_b_opt {
        result.push(result[0] + opk_b);
        result.push(result[1] + opk_b);
        result.push(result[2] + opk_b);
        result.push(result[3] + opk_b);
        result.push(result[4] + opk_b);
        result.push(result[5] + opk_b);
        result.push(result[6] + opk_b);
        result.push(result[7] + opk_b);
    }

    result
}

pub fn pqmudh_alice_with_prep<R: Rng + CryptoRng>(
    ...
    let exp_1_num: BigUint = (BigUint::from_bytes_le(&alpha_1_bytes) * &ik_a_private_num) % &group_order;
    let exp_2_num: BigUint = (BigUint::from_bytes_le(&alpha_2_bytes) * &ek_a_private_num) % &group_order;
    let exp_3_num: BigUint = (BigUint::from_bytes_le(&alpha_3_bytes) * &ek_a_private_num) % &group_order;
    ...
    let pre_computation = pre_compute_group_elements(ik_b, spk_b, opk_b_opt);
    ...
    for i in (0..31).rev() {
        for j in (0..7).rev() {
            ...
            shared_point += pre_computation[index];
